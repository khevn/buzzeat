...
def output():
	...
	try:
		if food == "": #for finding best trending dishes (no search term specified)
			similar_food = []
			final_posts = posts.sort('likes', ascending = False)[['image_url','lat','long','post_url','likes']].head(200)
		else:
			...
			final_posts = posts[posts['text'].str.contains(search_terms, na=False)]
	except:
	...
	try:
		...
		cols = bze.create_map([search_loc['lat'], search_loc['lng']], 
